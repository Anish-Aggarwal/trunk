...
function ConfiguringBizTalkServerHostAndHostInstances
{
	...
	[string]$receiveHostName = 'CmrsReceiveHost'  
	[string]$sendHostName = 'CmrsSendHost'
	[string]$processingHostName = 'CmrsProcessingHost' # use this to create another processing host
	[string]$trackingHostName = 'CmrsTrackingHost'
    [string]$PreProcessingHostName = 'CmrsPreProcessingHost'
    [string]$TnTHostName = 'CmrsTnTHost'
	# Note: why we need to create 32bits hosts? FTP, POP3 and SQL doesn't support 64bits 
	[string]$receive32HostName = 'CmrsReceiveHost'
	[string]$send32HostName = 'CmrsSendHost'
    ...
    # Create a host for tracking
	CreateBizTalkHost $PreProcessingHostName 1 $ntHostGroupName $false $false $false
	
	# Create a host for orchestrations
	CreateBizTalkHost $TnTHostName 1 $ntHostGroupName $false $false $false
 
	# Create a host instance for orchestrations
	CreateBizTalkHostInstance $TnTHostName $bizTalkServerName $hostCredentials.UserName $hostCredentialsPassword
 
	# Create a host instance for tracking
	CreateBizTalkHostInstance $PreProcessingHostName $bizTalkServerName $hostCredentials.UserName $hostCredentialsPassword
}
